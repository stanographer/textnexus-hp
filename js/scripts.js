...
jQuery(document).ready(function($) {
	...
	$('.albums__item').on('click', function(event) {
	...
	$('.js-video-target').on('click', function(event) {
		event.preventDefault();
	...
	function subForm(f){
		...
		if (!hasErrors) {
			$.post('mail.php', $form.serialize(), function(data){
				if (data == 1) { $respond = msg.success }
				else if (data == 0) { $respond =  msg.error }
				else { $respond = msg.fail }
				$form.find('.js-respond').text( $respond );
			}).fail(function(response) {
				console.log(response);
				UIkit.modal.dialog('<p class="uk-modal-body c-dark">'+msg.fail+'</p>');
			});
		}
		$form.find('.js-respond').text( $respond );
		return false;
	...
	$.instagramFeed({
		...
		callback: function(data){
			var items = data.edge_owner_to_timeline_media.edges,
				...
				code = '';
			for (var i = 0; i < items_max_count; i++) {
				code += '<div><a href="https://www.instagram.com/p/'+items[i].node.shortcode+'" target="_blank" class="cover__photo uk-background-cover uk-transition-toggle '+((i == 2) ? 'uk-visible@s' : (i == 3 || i == 4) ? 'uk-visible@m' : '' )+'" src="'+items[i].node.thumbnail_src+'" data-src="'+items[i].node.thumbnail_src+'" data-uk-img><span class="cover__link uk-box-shadow-large fab fa-instagram uk-transition-scale-up"></span></a></div>';
			}
			$('#instafeed').html(code);
		}
	});
	// Use the code below instead of first one for view images by hashtag
	/* $.instagramFeed({
		// Your hashtag here:
		tag: 'musicband',
		get_data: true,
		callback: function(data){
			var items = data.edge_hashtag_to_media.edges,
				items_max_count = (items.length < 5) ? items.length : 5,
				code = '';
			for (var i = 0; i < items_max_count; i++) {
				code += '<div><a href="https://www.instagram.com/p/'+items[i].node.shortcode+'" target="_blank" class="cover__photo uk-background-cover uk-transition-toggle '+((i == 2) ? 'uk-visible@s' : (i == 3 || i == 4) ? 'uk-visible@m' : '' )+'" src="'+items[i].node.thumbnail_src+'" data-src="'+items[i].node.thumbnail_src+'" data-uk-img><span class="cover__link uk-box-shadow-large fab fa-instagram uk-transition-scale-up"></span></a></div>';
	...
	UIkit.util.on('.js-modal-article', 'show', function() {
		var modal = $(this);
		function scrollBlockMaxH(){
			if (modal.find('[class^="cover__photo"]') && modal.find('.js-scrollbar')){
				var img = modal.find('[class^="cover__photo"]'),
					h = img.innerHeight(),
					scb = modal.find('.js-scrollbar'),
					scbParent = scb.parent(),
					parentPadTop = parseInt(scbParent.css('padding-top')),
					parentPadBottom = parseInt(scbParent.css('padding-bottom'));
				modal.find('.js-scrollbar').css('max-height', h - parentPadTop - parentPadBottom);
			}
		}
		scrollBlockMaxH();
		window.resizeCSB = wnd.on('resize', scrollBlockMaxH);
	});
